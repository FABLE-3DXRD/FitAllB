def FCN(inp):	
	"""
	Function to build fcn.py, the module containing the functions to be minimised by Minuit.
	This is necessary because Minuit must be called with a function where all refinable parameters 
	are given in the function call, thus strings/arrays are not allowed.
	fcn.py contains least squares expresions for refining the difference between experimental and calculated g-vectors
	...
	These are all called with the following refinable parameters:
		...
		for each grain: x,y,z,phia,PHI,phib,epsaa,epsab,epsac,epsbb,epsbc,epscc
	...
	Additional needed parameters that are written in fcn.py
		...
		nrefl[i]:	list of number of assigned reflections for each grain, i=range(no_refl)
		id[i][j]:	list of peak ids for each assigned reflection, i=range(no_refl) and j=range(nrefl[i])
		h[i][j]:
		k[i][j]:	list of h,k,l for each assigned reflection, i=range(no_refl) and j=range(nrefl[i])
		l[i][j]:
		w[k]:		omega for each measured reflection, id[i][j] used for identification, k=range(max(id[i][j])) 
		dety[k]:	dety for each measured reflection, id[i][j] used for identification, k=range(max(id[i][j])) 
		detz[k]:	detz for each measured reflection, id[i][j] used for identification, k=range(max(id[i][j])) 
		vars[k]:	error vector of gexp for each measuered reflection, id[i][j] used for identification, k=range(max(id[i][j])) 
	...
	string = string + "id = [\n" 
	for i in range(inp.no_grains):
		string = string + '\t[%i' %inp.id[i][0]
		for j in range(1,inp.nrefl[i]):
	...
	# w
	string = string + "w = [%f" %inp.w[0]
	nn = 1
	for i in range(1,inp.param['total_refl']):
		nn = nn + 1
	...
	# dety
	string = string + "dety = [%f" %inp.dety[0]
	nn = 1
	for i in range(1,inp.param['total_refl']):
		nn = nn + 1
	...
	# detz
	string = string + "detz = [%f" %inp.detz[0]
	nn = 1
	for i in range(1,inp.param['total_refl']):
		nn = nn + 1
...
# helpful functions
	...
	string = string + '\t Omega = tools.quart2Omega(w,wx,wy)\n'
...
# FCNpeak function calculating the contribution from a single peak including all variables	
	...
	f = open('./%s/fcn.py' %inp.fit['stem'],'w')
	f.write(string)
	f.close()
