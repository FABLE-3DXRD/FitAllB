...
def mean_ia(inp,limit,only=None):
        ...
        for i in range(inp.no_grains):
            if i+1 in inp.fit['skip']:
                ...
                no = 0
                for k in range(inp.fit['no_det']-1,-1,-1):
                    no = no + inp.nrefl[k][i]
                for k in range(inp.fit['no_det']-1,-1,-1):
                    for j in range(inp.nrefl[k][i]-1,-1,-1):
                        no = no - 1
                        Omega = tools.form_omega_mat_general(inp.w[k][inp.id[k][i][j]]*n.pi/180,inp.values['wx']*n.pi/180,inp.values['wy']*n.pi/180)
                        gexp = fcn.gexp(inp.w[k][inp.id[k][i][j]],inp.dety[k][inp.id[k][i][j]],inp.detz[k][inp.id[k][i][j]],
                                    inp.values['wx'],inp.values['wy'],inp.values['tx%s' %k],inp.values['ty%s' %k],inp.values['tz%s' %k],
                                    inp.values['py%s' %k],inp.values['pz%s' %k],inp.values['cy%s' %k],inp.values['cz%s' %k],inp.values['L%s' %k],
                        ...
                        gcalc = fcn.gcalc(inp.values['a'],inp.values['b'],inp.values['c'],inp.values['alpha'],inp.values['beta'],inp.values['gamma'],
                                      inp.h[k][i][j],inp.k[k][i][j],inp.l[k][i][j],
                                      rod[0],rod[1],rod[2],
...
#                    gcalc = n.dot(Omega,gcalc)
                        inp.mean_ia[i][no] = IA(n.transpose(gexp)[0],n.transpose(gcalc)[0])
#                    print i+1,inp.mean_ia[i][j]
                        if inp.mean_ia[i][no] > limit:
                            delete = delete + 1
                            reject(inp,i,j,k,'ia')
                ...
                for k in range(inp.fit['no_det']): 
                    for j in range(inp.nrefl[k][i]): 
                        inp.residual[i][j] = fcn.peak(inp.values['a'],inp.values['b'],inp.values['c'],inp.values['alpha'],inp.values['beta'],inp.values['gamma'],
                                              inp.h[k][i][j],inp.k[k][i][j],inp.l[k][i][j],
                                              inp.w[k][inp.id[k][i][j]],inp.dety[k][inp.id[k][i][j]],inp.detz[k][inp.id[k][i][j]],
                                              #n.array([inp.Syy[inp.id[i][j]],inp.Szz[inp.id[i][j]],inp.Sww[inp.id[i][j]]]),
                                              inp.vars[k][i][j], 
                                              inp.values['wx'],inp.values['wy'],
                                              inp.values['tx%s' %k],inp.values['ty%s' %k],inp.values['tz%s' %k],
                                              inp.values['py%s' %k],inp.values['pz%s' %k],
                                              inp.values['cy%s' %k],inp.values['cz%s' %k],
                                              inp.values['L%s' %k],
                                              inp.values['x%s' %i],inp.values['y%s' %i],inp.values['z%s' %i], 
...
#                print i, len(data[i]),medi,mean,'\n',data[i],'\n'
        ...
        for i in range(inp.no_grains):
            if i+1 in inp.fit['skip'] or i+1 not in only:
                ...
                for k in range(inp.fit['no_det']-1,-1,-1): 
                    no = no + inp.nrefl[k][i]
                for k in range(inp.fit['no_det']-1,-1,-1): # loop backwards to make pop work
                    for j in range(inp.nrefl[k][i]-1,-1,-1): # loop backwards to make pop work
                        no = no - 1
                        if inp.residual[i][no] > maxres[i]:
                            delete = delete + 1
...
def reject(inp,i,j,k,message):
        """
        Reject peak j of detector k from grain i and print the message as rejectvalue
        """
        inp.vars[k][i].pop(j)
        inp.fit['rejectgrain'].append(i)
        inp.fit['rejectdet'].append(k)
        inp.fit['rejectid'].append(inp.id[k][i].pop(j))
        inp.fit['hh'].append(inp.h[k][i].pop(j))
        inp.fit['kk'].append(inp.k[k][i].pop(j))
        inp.fit['ll'].append(inp.l[k][i].pop(j))
        inp.fit['rejectvalue'].append(message)
        inp.nrefl[k][i] = inp.nrefl[k][i] - 1
        inp.fit['outliers'] = inp.fit['outliers'] + 1
        inp.newreject = inp.newreject + 1
        inp.fit['newreject_grain'].append(i+1)
        no = 0
        for kk in range(k-1):
            no = no + inp.nrefl[kk][i]
        inp.residual[i].pop(no+j) 
        inp.volume[i].pop(no+j)
        inp.mean_ia[i].pop(no+j)
